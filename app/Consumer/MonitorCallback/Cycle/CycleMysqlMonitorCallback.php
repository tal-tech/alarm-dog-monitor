...
class CycleMysqlMonitorCallback extends ConsumerCallback
{
    /**
    ...
    private function alarm($msgs, $options = [])
    {
        Coroutine::create(function () use ($msgs, $options) {
            $this->stdOutLogger->info('cycle mysql callback msg:' . json_encode($msgs));
            ...
            $this->stdOutLogger->info('cycle mysql alarm condition:' . json_encode($alarmCondition));
            //查询老数据做对比
            ...
            foreach ($alarmCondition as $key => $arr) {
                $this->stdOutLogger->info('【cycle mysql monitor】data source ' . $taskModel->datasource_id . ', cycle mysql validate data ------msg:' . json_encode($msgs) . '-----rule:' . json_encode($arr['rule']) . '-----olddata:' . json_encode($oldDataArr));
